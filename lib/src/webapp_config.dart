...
class WebappConfig {
  ...
  WebappWidgetFunction windowsTitle = _webappWinTitle;
  WebappWidgetFunction appTitle = _webappAppTitle;
  WebappWidgetFunction navigation = _webappNavigation;
  ...
  Future<void> _initWindows() async {
    ...
    if (isWindows()) {
      JavaScriptHandler.add('fullscreen', (controller, data) {
        if (data is bool) {
          controller.setFullScreen(data);
        }
        return null;
      });
    }
...
typedef WebappWidgetFunction = Widget Function(WebappController controller);

typedef WebappBottomFunction = PreferredSize Function(
    WebappController controller);

Widget _webappWinTitle(WebappController controller) {
      ...
      builder: (context, value, child) {
        ...
        return Row(
          ...
          children: [
            ...
            webappConfig.navigation(controller),
          ],
        );
      });
}

Widget _webappAppTitle(WebappController controller) {
  return ValueListenableBuilder(
      valueListenable: controller,
      builder: (context, value, child) {
        List<Widget> titles = [];
...
Widget _webappNavigation(WebappController controller) {
